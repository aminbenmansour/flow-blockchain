...
transaction(
    ...
    prepare(signer: AuthAccount) {
        self.mintingIDBefore = BAYC.totalSupply

        // borrow a reference to the NFTMinter resource in storage
        self.minter = signer.borrow<&BAYC.NFTMinter>(from: BAYC.MinterStoragePath)
            ?? panic("Account does not store an object at the specified path")

        // Borrow the recipient's public NFT collection reference
        self.recipientCollectionRef = getAccount(recipient)
            .getCapability(BAYC.CollectionPublicPath)
    ...
    post {
        ...
        BAYC.totalSupply == self.mintingIDBefore + 1: "The total supply should have been increased by 1"
    }
}
